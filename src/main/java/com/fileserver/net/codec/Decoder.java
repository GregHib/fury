package com.fileserver.net.codec;

import com.fileserver.FileServerConstants;
...
public class Decoder extends ByteToMessageDecoder {
	...
	private void decodeOnDemandRequest(ChannelHandlerContext ctx, ByteBuf in) {
		...
		if (in.readableBytes() >= Byte.BYTES + Integer.BYTES) {
			...
			//Read file type
			int fileType = in.readUnsignedByte();
			//Read file id
			...
			ByteBuf file = null;
			try {
				file = GameLoader.getCache().getFile(fileType + 1, fileId);
			} catch (Exception e) {
				System.out.println("File error: " + fileId + " " + fileType);
				//e.printStackTrace();
			}
			...
			if(file != null) {
                ...
                System.out.println("Missing file: " + fileId + " " + fileType);
                ctx.close();
			}
		}
	}
