...
public class CockatriceCombat extends CombatScript {
	...
	@Override
	public Object[] getKeys() { return new Object[]{ 1620 }; }
	...
	public int attack(Mob mob, final Figure target) {
		...
		if (!Slayer.hasReflectiveEquipment(target)) {
			...
			delayHit(mob, projectile.getTickDelay(), target, getMagicHit(mob, getRandomMaxHit(mob, mob.getCombatDefinition().getAttackStyle(), target)));
