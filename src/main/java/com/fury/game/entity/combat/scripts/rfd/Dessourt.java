...
public class Dessourt extends CombatScript {
    ...
    public int attack(Mob mob, Figure target) {
        ...
        if (Utils.isOnRange(target, mob, 0))
            ...
            if(Misc.random(8) == 0)
                mob.forceChat("Hssssssssssssss");
            Projectile projectile = new Projectile(mob, target, 596, 44, 30, 35, 3, 10, 0);
            ProjectileManager.send(projectile);
            delayHit(mob, projectile.getTickDelay(), target, getMagicHit(mob, getRandomMaxHit(mob, 100, MobCombatDefinitions.MAGE, target)));
        }
