package com.fury.game.entity.character;
 ...
 * lookups needed to add a new figure.
 *
 * @param <E>
 *            the type of figure being managed with this collection.
 * @author lare96 <http://github.com/lare96>
 */
public final class CharacterList<E extends Figure> implements Iterable<E> {

	/**
	 * The backing array of {@link java.lang.Character}s within this collection.
	 */
	private E[] characters;

	/**
	 * The queue containing all of the slots that {@link java.lang.Character}s were
	 * recently removed from. This is used to reduce slot lookup times for
	 * characters being added to this figure list.
	 */
	private final Queue<Integer> slotQueue = new ConcurrentLinkedQueue<>();
	...
	public CharacterList(int capacity) {
		...
		this.characters = (E[]) new Figure[capacity];
		this.size = 0;
	}
	...
	public boolean remove(E e) {
		...
		// A player cannot be removed from the figure list unless it's from a
		// logout request. On-demand removes for players completely mess up the
		// login process, so we request a logout instead.
		if (e.isPlayer()) {
			Player player = (Player) e;
			if (player.getSession().isPresent() && player.getSession().get().getState() != SessionState.LOGGING_OUT) {
				player.logout();
				return false;
			}
		}
	...
	public void clear() {
		forEach(this::remove);
		characters = (E[]) new Figure[capacity];
		size = 0;
	}
	 ...
	 *
	 * @param <E>
	 *            the type of figure being iterated over.
	 * @author lare96 <http://github.com/lare96>
	 */
	private static final class CharacterListIterator<E extends Figure> implements Iterator<E> {
