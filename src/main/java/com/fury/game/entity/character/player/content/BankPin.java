...
public class BankPin {
    ...
    public static void deletePin(Player player) {
        if (player.getBankPinAttributes().hasBankPin() && !player.getBankPinAttributes().hasEnteredBankPin()) {
            player.message("Please enter your pin before doing that.");
            BankPin.init(player);
            return;
        }
        ...
        player.message("Your bank-pin was deleted.");
    ...
    public static void init(Player player, Runnable after) {
        if (player.getBankPinAttributes().getInvalidAttempts() == 3) {
            if (System.currentTimeMillis() - player.getBankPinAttributes().getLastAttempt() < 400000) {
                player.message("You must wait " + (int) ((400 - (System.currentTimeMillis() - player.getBankPinAttributes().getLastAttempt()) * 0.001)) + " seconds before attempting to enter your bank-pin again.");
                return;
            } else
        ...
        if (after != null)
            player.getTemporaryAttributes().put("after-pin", after);
    ...
    public static void clickedButton(Player player, int button) {
        ...
        if (player.getBankPinAttributes().getEnteredBankPin()[0] == -1) {
            ...
            if (!player.getBankPinAttributes().hasBankPin()) {
                ...
                Achievements.finishAchievement(player, Achievements.AchievementData.SET_A_BANK_PIN);
                player.message("You've created a bank-pin. Your digit is " + player.getBankPinAttributes().getEnteredBankPin()[0] + "-" + player.getBankPinAttributes().getEnteredBankPin()[1] + "-" + player.getBankPinAttributes().getEnteredBankPin()[2] + "-" + player.getBankPinAttributes().getEnteredBankPin()[3] + ". Please write it down.");
                player.getPacketSender().sendInterfaceRemoval();
                return;
            }
            for (int i = 0; i < player.getBankPinAttributes().getEnteredBankPin().length; i++) {
                if (player.getBankPinAttributes().getEnteredBankPin()[i] != player.getBankPinAttributes().getBankPin()[i]) {
                    ...
                    player.message("Invalid bank-pin entered entered.");
                    return;
                }
            }
            player.getBankPinAttributes().setInvalidAttempts(0).setHasEnteredBankPin(true);
            Object after = player.getTemporaryAttributes().remove("after-pin");
            player.getPacketSender().sendInterfaceRemoval();
            if (after != null)
                ((Runnable) after).run();
        }
