...
public class PlayerRelations {
	...
	public PlayerRelations updateLists(boolean online) {
	    return updateLists(online, false);
    }

	public PlayerRelations updateLists(boolean online, boolean login) {
		...
		for (Player players : GameWorld.getPlayers()) {
			if(players == null)
				continue;
			if(players.getInterfaceId() == 40172)
				ClanChatManager.sendFriendsList(player);
			boolean temporaryOnlineStatus = online;
			if (players.getRelations().getFriendList().contains(player.getLongUsername())) {
				...
				players.getPacketSender().sendFriend(player.getLongUsername(), temporaryOnlineStatus ? 1 : 0, false);
			}
			boolean tempOn = true;
			if (player.getRelations().getFriendList().contains(players.getLongUsername())) {
				...
				player.getPacketSender().sendFriend(players.getLongUsername(), tempOn ? 1 : 0, login);
			}
		}
		return this;
	}
	...
	public void sendFriends(boolean login) {
		for(int i = 0; i < player.getRelations().getFriendList().size(); i++) {
			player.getPacketSender().sendFriend(player.getRelations().getFriendList().get(i), 0, login);
		}
	}
	...
	public void addFriend(Long username) {
		...
		if (friendList.contains(username)) {
			...
			Achievements.finishAchievement(player, Achievements.AchievementData.ADD_A_FRIEND);
			...
			if (friend != null) {
				...
				if(player.getCurrentClanChat() != null) {
					if(player.getCurrentClanChat().getOwnerName().equals(player.getUsername()))
						player.getCurrentClanChat().setOwner(player);
	...
	public void message(Player friend, byte[] message, int size) {
		if(friend == null || message == null) {
			this.player.message("This player is currently offline.");
			return;
		}

		if(Punishment.isMuted(player) || Punishment.isHardwareMuted(player)) {
			player.message("You are muted and cannot pm people.");
			return;
		}
		...
		player.getLogger().addPrivateMessage(NameUtils.decode(new ByteBuffer(message), message.length), friend);
		friend.getPacketSender().sendPrivateMessage(player.getLongUsername(), player.getRightsId(), message, size);
	}
