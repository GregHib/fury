...
public class CheckClanLogsCommand implements Command {

    private static Pattern pattern = Pattern.compile("^check\\sclan\\slogs\\s(.*)");
    ...
    public String prefix() {
        return "check clan logs ";
    }

    @Override
    public String format() {
        return "check clan logs [username]";
    }
    ...
    public void process(Player player, String... values) {
        ...
        for(LoggedMessage message : target.getLogger().getClanMessages())
            list.add(message.getTime() + " [" + message.getTo() + "] " + message.getMessage());

        ListWidget.display(player, "Clan Chat Logs: " + target.getUsername(), "", list.toArray(new String[list.size()]));
    }
