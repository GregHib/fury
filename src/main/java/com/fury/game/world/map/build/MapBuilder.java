...
public class MapBuilder {
    ...
    public static void init() {
        for (int regionId : FORCE_LOAD_REGIONS)
            GameWorld.getRegions().load(regionId);
    }

    private static final Object ALGORITHM_LOCK = new Object();
    public static final List<Integer> EXISTING_MAPS = new ArrayList<>();
    ...
    public static int findEmptyRegionHash(int widthChunks, int heightChunks) {
        //16, 16
        //8 rooms by 8 rooms
        //every region is 64x64 = 4 rooms by 4 rooms
        ...
        synchronized (ALGORITHM_LOCK) {
            for (int regionX = 2; regionX <= MAX_REGION_X - regionsDistanceX; regionX++) {
                skip:
                for (int regionY = 2; regionY <= MAX_REGION_Y - regionsDistanceY; regionY++) {
                    int regionHash = getRegionId(regionX, regionY); // map hash because skipping to next map up
                    for (int checkRegionX = regionX; checkRegionX <= regionX + regionsDistanceX; checkRegionX++) {
                        for (int checkRegionY = regionY; checkRegionY <= regionY + regionsDistanceY; checkRegionY++) {
    ...
    public static DynamicRegion createDynamicRegion(int regionId) {
        synchronized (ALGORITHM_LOCK) {
            ...
            GameWorld.getRegions().getActiveRegions().put(regionId, newRegion);
            return newRegion;
        }
    }

    public static void reloadDynamicRegion(int regionId) {
        Region region = GameWorld.getRegions().getActiveRegions().get(regionId);
        if (region != null) {
            for (int i = 0; i < 4; i++) {
                Collection<Player> players = region.getPlayers(i);
    ...
    public static void destroyRegion(int regionId) {
        ...
        if (region != null) {
            for (int i = 0; i < 4; i++) {
                Collection<Player> players = region.getPlayers(i);
                Collection<Mob> mobs = region.getNpcs(i);
                if (region.getFloorItems(i) != null)
                    region.getFloorItems(i).clear();
                if (region.getSpawnedObjects() != null) {
                    region.getRemovedOriginalObjects().clear();
                    for (GameObject obj : region.getSpawnedObjects())
                ...
                for (Mob mob : mobs) {
                    if (mob == null)
                        continue;
                    if (mob.isSummoningNpc()) {
                        mob.setPosition(0, 0, 0); // sets 0,0,0 and forces to teleto player and not force load region and loss space
                        continue;
                    }
                    mob.deregister();
                }
                GameWorld.getRegions().getActiveRegions().remove(regionId);
