...
public final class Dungeon {
	...
	public static void main(String[] args) {
		...
		while (true) {
			System.out.println("TOOK: " + (Misc.currentTimeMillis() - lastDung) + ", ");
		...
		for (int y = 0; y < map[0].length; y++) {
			for (int x = 0; x < map.length; x++) {
				if (map[x][y] != null) {
					...
					//g.setColor(Color.getHSBColor((map[worldX][worldY].getCreationIndex() * 4f) / 360f, 1.0f, 0.8f));
...
//					if (map[worldX][worldY].getCreationIndex() == 0) {
//						g.setColor(Color.darkGray);
//						g.fillRect(worldX * 20 + 27, (7 - worldY) * 20 + 47, 6, 6);
//
//					}
...
//					int tier = map[worldX][worldY].getCreationIndex();// == creationCount ? branchCount - 1 : creationBranchReference[map[worldX][worldY].getCreationIndex()];
					g.setFont(new Font("TimesRoman", Font.BOLD, 7));
					g.setColor(Color.green);
//					g.drawString("\033" + tier, worldX * 20 + 36, (7 - worldY) * 20 + 49);

					for (int l = 0; l < map[x][y].getRoom().getDoorDirections().length; l++) {
						int lock0 = 0;//(map[worldX][worldY].getDoorTypes()[l] >> 16 & 0xFFFF) - 50208;
	...
	public Dungeon(DungeonManager manager, int floorId, int complexity, int size) {
		try {
			...
			for (RoomNode node : children) {
				...
				/*else if (entity.children.size() > 0 && entity.children.stream().allMatch(c -> c.lock == -1) && puzzleChance > random.nextDouble()) {
					puzzle = true;
					possibilities = DungeonUtils.selectPossibleRooms(DungeonConstants.PUZZLE_ROOMS, complexity, type, entity.north(), entity.east(), entity.south(), entity.west(), entity.rotation());
				}*/
				else {
				...
				if (possibilities.length <= 0) {
					manager.getParty().getLeaderPlayer().message("This floor is currently disabled.");
					manager.getParty().getLeaderPlayer().getDungManager().tickOff(manager.getParty().getFloor());
					ready = false;
					break;
				} else
					rand = random.nextInt(possibilities.length);
				map[node.x][node.y] = possibilities[rand];
				if (node.isCritPath) {
					critCount++;
					...
					if (neighbor.parent == node) {
						if (puzzle) {
							...
							if (sd.getClosedObject(type) == -1) //some frozen skill doors dont exist
								continue;
							int level = manager == null ? 1 : neighbor.isCritPath ? ((sd.getSkill() == Skill.PRAYER ? manager.getParty().getMaxLevel(sd.getSkill()) / 10 : manager.getParty().getMaxLevel(sd.getSkill())) - random.nextInt(10)) : random.nextInt(sd.getSkill() == Skill.SUMMONING || sd.getSkill() == Skill.PRAYER ? 100 : 106);
