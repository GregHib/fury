...
public class DungeonUtils {
    ...
    public static int getBindedId(Item item) {
        ...
        for (int i = 15775; i <= 16272; i++) {
            if (Loader.getItem(i).getName().replace(" (b)", "").equals(name))
                return i;
        }
    ...
    public static int getSafeMusic(int type) {
        return DungeonConstants.SAFE_MUSICS[type][Misc.random(DungeonConstants.DANGEROUS_MUSICS[type].length)];
    }

    public static int getDangerousMusic(int type) {
        return DungeonConstants.DANGEROUS_MUSICS[type][Misc.random(DungeonConstants.DANGEROUS_MUSICS[type].length)];
    }

    public static RoomReference getRandomRoom(Room[][] map) {
        return new RoomReference(Misc.random(map.length), Misc.random(map[0].length));
    }

    public static int getGuardianCreature(int type) {
        return DungeonConstants.GUARDIAN_CREATURES[type + 1][Misc.random(DungeonConstants.GUARDIAN_CREATURES[type + 1].length)];
    }

    public static int getGuardianCreature() {
        return DungeonConstants.GUARDIAN_CREATURES[0][Misc.random(DungeonConstants.GUARDIAN_CREATURES[0].length)];
    }

    public static int getForgottenWarrior() {
        int type = Misc.random(DungeonConstants.FORGOTTEN_WARRIORS.length);
        return DungeonConstants.FORGOTTEN_WARRIORS[type][Misc.random(type)];
    }

    public static int getHunterCreature() {
        return DungeonConstants.HUNTER_CREATURES[Misc.random(DungeonConstants.HUNTER_CREATURES.length)];
    }

    public static int getSlayerCreature() {
        return DungeonConstants.SLAYER_CREATURES[Misc.random(DungeonConstants.SLAYER_CREATURES.length)];
    }
    ...
    public static int getBranches(int tier) {
    ...
    public static int getMaxTier(DungeonPartyManager party, int cap, Skill... skills) {
        int maxTier = cap;
