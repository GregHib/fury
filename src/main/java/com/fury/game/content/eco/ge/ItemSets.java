...
public class ItemSets {
    ...
    public static void exchangeSets(Player player, Sets set) {
        if (set != null) {
            if(set.getItems().length > player.getInventory().getSpaces()) {
                player.message("You need " + set.getItems().length + " free inventory items to open this set.");
                return;
            }
            for (int exchangedId : set.getItems())
                player.getInventory().add(new Item(exchangedId, 1));
            player.getInventory().delete(new Item(set.getId(), 1));
        }
    }
}
