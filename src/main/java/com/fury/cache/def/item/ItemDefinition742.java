...
public class ItemDefinition742 extends ItemDefinition {
    public static ItemDefinition742[] cache;
    protected static ByteBuffer buffer;
    protected static int[] indices;
    public static int total;

    public ItemDefinition742(int id) {
        super(id);
    ...
    public void load(CacheArchive archive, String name) {
        ...
        cache = new ItemDefinition742[total];
    }

    public ItemDefinition742 forId(int id) {
        ItemDefinition742 definition = id < 0 || id >= cache.length ? null : cache[id];
        if(definition == null) {
            definition = new ItemDefinition742(id);
    ...
    public void readValues(ByteBuffer buffer, int opcode) {
        if (opcode == 1) {
            ...
            equipType2 = buffer.readUnsignedByte();
            ...
            stackRevisions[opcode - 100] = Revision.PRE_RS3;
        } else if (opcode == 110) {
            ...
            int maleWieldX = (byte) (buffer.readByte() << 0);
            int maleWieldY = (byte) (buffer.readByte() << 0);
            int maleWieldZ = (byte) (buffer.readByte() << 0);
        } else if (opcode == 126) {
            int femaleWieldX = (byte) (buffer.readByte() << 0);
            int femaleWieldY = (byte) (buffer.readByte() << 0);
            int femaleWieldZ = (byte) (buffer.readByte() << 0);
        ...
        } else if (opcode == 157) {//new
            boolean aBool7955 = true;
        } else if (opcode == 161) {//new
            int anInt7904 = buffer.readUnsignedShort();
        } else if (opcode == 162) {//new
            int anInt7923 = buffer.readUnsignedShort();
        } else if (opcode == 163) {//new
            int anInt7939 = buffer.readUnsignedShort();
        } else if (opcode == 164) {//new coinshare shard
            String aString7902 = buffer.readString();
