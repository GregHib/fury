...
public final class ByteBuffer {
	...
	public String getString() {
		...
		return new String(buffer, index, position - index - 1);
	}
	...
	public final int getMedium() {
		position += 3;
		return (0xff & buffer[position - 1]) + ((buffer[position - 3] << 16 & 0xff0000) + (0xff00 & buffer[position - 2] << 8));
	}
	...
	public int readUnsignedTribyte() {//getMedium
		position += 3;
	...
	public int readUnsignedSmart() {
		int i = 255 & buffer[position];
		return i >= 128 ? readUnsignedShort() - 32768 : readUnsignedByte();
	}
